...
const SkyscannerSearch = ({ variant = "hero", className }: SkyscannerSearchProps) => {
  ...
  return (
    <div className={containerCls}>
      ...
      <div className={cn("flex items-center gap-1 p-3 border-b", isHero ? "border-white/10" : "border-gray-100")}>
        {([
          { id: "flights", icon: Plane, label: "رحلات طيران" },
          { id: "hotels", icon: Building2, label: "فنادق" },
          { id: "cars", icon: Car, label: "السيارات" },
          ...
          <>
            ...
            <div className={cn("flex items-center gap-3 flex-wrap", isBanner ? "pb-3" : "")}>
              {([
                ...
                { value: "multicity", label: "متعدد المدن" },
              ] as { value: TripType; label: string }[]).map(({ value, label }) => (
                <button
                  key={value}
...
const PassengersDropdown = ({
  ...
  cabinClass: string;
  cabinLabels: Record<string, string>;
  onAdjust: (key: keyof PassengerCount, delta: number) => void;
  onCabinChange: (c: "ECONOMY" | "PREMIUM_ECONOMY" | "BUSINESS" | "FIRST") => void;
  onClose: () => void;
  isHero: boolean;
}) => {
