  ...
  return (
    <section className="relative h-screen overflow-hidden">
      ...
      <div className="absolute inset-0">
        ...
        {/* Animated Shapes */}
        <div className="absolute top-20 right-20 w-96 h-96 bg-secondary/10 rounded-full blur-3xl animate-float [animation-delay:0s]" />
        <div className="absolute bottom-20 left-20 w-80 h-80 bg-gold/10 rounded-full blur-3xl animate-float [animation-delay:2s]" />
        <div className="absolute top-1/2 right-1/3 w-64 h-64 bg-accent/5 rounded-full blur-3xl animate-float [animation-delay:4s]" />
      </div>
      ...
      <div className="container relative h-full flex items-center">
        <div className="grid lg:grid-cols-2 gap-12 items-center w-full">
          ...
          <div className="text-primary-foreground space-y-8 animate-fade-in">
            {/* Premium Badge */}
            <div className="inline-flex items-center gap-2 bg-secondary/20 backdrop-blur-sm px-6 py-3 rounded-full border border-secondary/30">
            ...
            <div>
              <h1 className="text-5xl md:text-7xl font-bold leading-tight mb-4">
                {currentData.title}
                <span className="block text-transparent bg-clip-text bg-gradient-to-r from-secondary via-gold to-gold-light animate-shimmer">
            ...
            <div className="flex flex-wrap gap-4">
              <Link to="/destinations">
                <Button 
                  size="lg"
                  className="bg-secondary hover:bg-secondary/90 text-secondary-foreground px-8 py-6 text-lg rounded-full shadow-gold group"
                >
          ...
          <div className="hidden lg:grid grid-cols-2 gap-4">
            {[
              { icon: MapPin, title: "وجهات متنوعة", desc: "أكثر من 50 وجهة حول العالم", color: "bg-secondary/20" },
              { icon: Calendar, title: "برامج مخصصة", desc: "خطط رحلتك كما تريد", color: "bg-gold/20" },
              { icon: Users, title: "خدمة 24/7", desc: "فريق دعم متاح دائماً", color: "bg-accent/20" },
              { icon: Shield, title: "حجز آمن", desc: "ضمان استرداد كامل", color: "bg-primary/20" },
            ].map((feature, index) => (
              <div
                key={index}
                className={cn(
      ...
      <div className="absolute bottom-8 right-8 animate-bounce">
        <div className="w-6 h-10 border-2 border-primary-foreground/30 rounded-full p-1">
          <div className="w-1 h-3 bg-secondary rounded-full mx-auto animate-pulse" />
        </div>
      </div>
