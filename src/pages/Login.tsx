...
const Login = () => {
  ...
  const formContent = (
    <>
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="space-y-2">
          ...
          <div className="relative">
            ...
            <Input id="email" name="email" type="email" placeholder="admin@traveliun.com" value={formData.email} onChange={handleInputChange} className="pr-10 h-12 rounded-xl" dir="ltr" />
          </div>
        </div>
        ...
        <div className="space-y-2">
          ...
          <div className="relative">
            ...
            <Input id="password" name="password" type={showPassword ? "text" : "password"} placeholder="••••••••" value={formData.password} onChange={handleInputChange} className="pr-10 pl-10 h-12 rounded-xl" dir="ltr" />
        ...
        <Button type="submit" className="w-full h-12 rounded-xl text-base" disabled={loading}>
          {loading ? "جاري تسجيل الدخول..." : (<>تسجيل الدخول<ArrowRight className="w-4 h-4 mr-2" /></>)}
        </Button>
      </form>
      ...
      <Button type="button" variant="outline" className="w-full h-12 rounded-xl" onClick={handleGoogleLogin}>
      ...
      <p className="text-center text-sm text-muted-foreground mt-4">
        ليس لديك حساب؟{" "}<Link to="/register" className="text-primary hover:underline font-medium">إنشاء حساب جديد</Link>
      </p>
      <p className="text-center text-sm text-muted-foreground mt-2">
        تحتاج مساعدة؟{" "}<Link to="/contact" className="text-primary hover:underline">تواصل معنا</Link>
      </p>
    </>
  );
  ...
  if (isMobile) {
    return (
      <PageLayout pageTitle="تسجيل الدخول" hideBackButton>
